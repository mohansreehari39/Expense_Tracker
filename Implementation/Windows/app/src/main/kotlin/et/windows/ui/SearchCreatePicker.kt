...
fun <T> SearchCreatePicker(
    ...
    Box(modifier) {
        ...
        DropdownMenu(
            ...
            // DropdownMenu defaults to a focusable popup (for keyboard
            // navigation of menu items), which steals keyboard focus from
            // the text field the instant it opens — every keystroke after
            // the first would go nowhere until clicking the field again.
            // This field is meant to keep receiving keystrokes while the
            // suggestions are open, so the popup must not be focusable;
            // items are still fully usable by mouse click.
